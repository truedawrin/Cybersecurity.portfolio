# 🛡️ TryHackMe Progression

Documenting my cybersecurity learning journey through TryHackMe rooms — focused on the Junior Penetration Tester / Red Team path.

<div align="center">

[![TryHackMe](https://img.shields.io/badge/TryHackMe-truedawrin-111827?style=for-the-badge&logo=tryhackme&logoColor=white)](https://tryhackme.com/p/truedawrin)
[![Status](https://img.shields.io/badge/Status-Active-3fb950?style=for-the-badge)]()

</div>

---

## 🎯 Current Focus

I am currently exploring both offensive and defensive security to build a well-rounded foundation, with my **main focus on the Junior Penetration Tester path**.

My goal is to document every room I complete, extracting the core concepts and commands to build a personal knowledge base.

---

## 📂 Progress Tracker

### 🛡️ Pre Security

| Room                           | Concept       |   Status    | Write-Up                                                 |
| :----------------------------- | :------------ | :---------: | :------------------------------------------------------- |
| Offensive Security Intro       | Red Team      | ✅ Complete | [Link](./Pre-Security/01.offensive-security-intro.md)    |
| Defensive Security Intro       | Blue Team     | ✅ Complete | [Link](./Pre-Security/01.defensive-security-intro.md)    |
| Carrers In Cyber               | Cybersecurity | ✅ Complete | [Link](./Pre-Security/03.carrers-in-cyber.md)            |
| Inside a computer system       | Systems       | ✅ Complete | [Link](./Pre-Security/03.inside-a-computer-system.md)    |
| Operating Systems introduction | Os            | ✅ Complete | [Link](./Pre-Security/04.operating-systems-introduction) |

### 🐧 Cyber Security 101

| Room                        | Concept |   Status    | Write-Up                                                  |
| :-------------------------- | :------ | :---------: | :-------------------------------------------------------- |
| Linux Fundamentals Part 1   | Linux   | ✅ Complete | [Link](./Cyber-Security-101/01.linux-fundamentals-pt1.md) |
| Windows Fundamentals Part 1 | Windows | ✅ Complete | [Link](./Cyber-Security-101/02.windows-fundamentals-1.md) |
| Windows Fundamentals Part 2 | Windows | ✅ Complete | [Link](./Cyber-Security-101/03.windows-fundamentals-2.md) |

### ⚔️ Jr Penetration Tester

| Room          | Concept |   Status    | Write-Up                                            |
| :------------ | :------ | :---------: | :-------------------------------------------------- |
| Search Skills | OSINT   | ✅ Complete | [Link](./Jr-penetration-tester/01.search-skills.md) |

---

## 🚀 Future Goals

- [ ] Complete the **Jr Penetration Tester** path.
- [ ] Begin the **Offensive Pentesting** path.
- [ ] Build a home lab for practicing Active Directory attacks.
- [ ] Obtain a certification (e.g., CompTIA Security+, eJPT).
